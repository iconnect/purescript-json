module Data.JSON
    ...
    , Pair(..), (.=), object
    , getKeyObj, lookupKeyObj, lookupDefaultObj, mkTuple
    ) where
...
getKeyObj :: forall a. (FromJSON a) => JObject -> String -> JParser a
getKeyObj obj key = case M.lookup key obj of
    Nothing -> Left $ "key " <> show key <> " not present"
    Just v  -> parseJSON v

infixr 5 getKeyObj as .:

lookupKeyObj :: forall a. (FromJSON a) => JObject -> String -> JParser (Maybe a)
lookupKeyObj obj key = case M.lookup key obj of
    Nothing -> pure Nothing
    Just v  -> parseJSON v

infixr 5 lookupKeyObj as .:?

lookupDefaultObj :: forall a. JParser (Maybe a) -> a -> JParser a
lookupDefaultObj pmval val = fromMaybe val <$> pmval

infixr 5 lookupDefaultObj as .!=
...
mkTuple :: forall a. (ToJSON a) => String -> a -> Pair
mkTuple name value = Tuple name (toJSON value)
